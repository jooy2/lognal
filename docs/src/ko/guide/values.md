...
수준, 그룹, 텍스트와 스타일, 단순한 값이 모두 같으면 똑같은 메시지로 봅니다. 오류나 자식이 있는 값을 담은 메시지는 합치지 않습니다.

이런 메시지를 어떻게 처리할지는 `mergeRepeats`로 정합니다.

| 값           | 동작                                                                                    |
| ------------ | --------------------------------------------------------------------------------------- |
| `true`       | 메시지를 버리고 앞 항목의 횟수를 올립니다. 기본값입니다.                                |
| `'collapse'` | 메시지를 모두 보관합니다. 연속된 메시지는 첫 항목과 횟수로 접혀 있고, 펼칠 수 있습니다. |
| `false`      | 메시지마다 항목을 따로 만듭니다.                                                        |
...
`'collapse'`이면 연속된 메시지가 접힌 채로 쌓입니다. `a a b a a`는 횟수 2인 `a`, `b`, 다시 횟수 2인 `a`로 보입니다. 횟수 배지를 클릭하면 접힌 메시지가 펼쳐지고, 한 번 더 클릭하면 다시 접힙니다. 항목 메뉴의 **반복 펼치기**와 **반복 접기**는 키보드와 터치 화면에서 같은 일을 합니다. 펼쳐 둔 묶음은 같은 메시지가 계속 들어와도 펼친 상태로 남습니다.

이 모드에서는 메시지 하나하나가 `maxEntries`에 들어가므로, 같은 메시지가 몰려도 다른 메시지와 똑같이 스토어를 채웁니다.
