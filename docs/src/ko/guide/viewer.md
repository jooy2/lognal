...
도구 모음의 **숨긴 메시지** 버튼을 누르면 규칙을 더하고 고치고 지우는 대화 상자가 열리며, 그동안 로그가 바로 따라 바뀝니다. 버튼에는 규칙이 가린 항목 수가 나옵니다. 코드에서는 `getMuteRules`, `setMuteRules`, `getMutedCount`, `openMuteDialog`로 같은 일을 하고, `toolbar: { mute: false }`를 넘기면 버튼이 사라집니다.
