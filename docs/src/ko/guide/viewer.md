...
웹에서는 오른쪽 클릭에 브라우저도 자기 메뉴를 띄우는데, Flutter는 이를 위젯 하나가 아니라 뷰 전체 단위로만 끌 수 있습니다. 그래서 뷰어는 오른쪽 클릭을 자기가 받는 항목 모드일 때만 브라우저 메뉴를 꺼 두고, 항목 모드인 뷰어가 없어지면 되돌립니다. 브라우저 메뉴를 그대로 쓰려면 뷰어를 만든 뒤 `BrowserContextMenu.enableContextMenu()`를 호출하세요. 항목 모드에서 오른쪽 클릭하면 메뉴가 둘 다 나옵니다.
