...
- **Work like a log viewer.** A toolbar follows new logs, clears the log, scrolls to the top or bottom, turns wrapping on and off, and filters by text and level. A status bar counts entries, each entry can show a timestamp, and text can be selected and copied. See [The viewer](/guide/viewer).
...
- **IME input on Safari.** Safari up to version 26 fires `compositionend` before the `keydown` of the key that commits a composition. The input line accounts for this, so pressing Enter to finish a Korean syllable does not submit the command.
