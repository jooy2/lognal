...
- `ViewerLabels` has seven new labels, `entryActions`, `copyEntry`, `copyEntryWithTime`, `copyEntryFormatted`, `copyEntryData`, `expandAll` and `collapseAll`. A complete `ViewerLabels` object of your own needs all seven. Overrides passed in `labels` are not affected.
